...
// value (if any) of moving the fold/tail into assembly. len(a) is read once (the
// two reads CSE to a single register anyway; this just makes that explicit).
func dot4AccPure(a, b []float32) float32 {
	n := len(a)
	if n == 0 {
		return 0
	}
	return dotNEON4AccPure(&a[0], &b[0], n)
}
