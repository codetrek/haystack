...
func CreateSimpleEngine(t *testing.T, query string, caseSensitive bool, maxWildLen, maxKwDist int) *SimpleContentSearchEngine {
	eng := NewSimpleContentSearchEngine(nil, maxWildLen, maxKwDist, false)
...
func TestParseQuerySimple(t *testing.T) {
	...
	runCase := func(t *testing.T, tt OneCase) {
		got := NewSimpleContentSearchEngine(nil, 4, 4, false)
...
func TestContentLineMatch(t *testing.T) {
	...
	// Test whole word matching using IsLineMatch to verify behavioral differences
	t.Run("Whole word matching - IsLineMatch behavior comparison", func(t *testing.T) {
		// Test case 1: Simple word that appears both as standalone and as substring
		// This simulates what would happen when searching with whole word vs prefix matching

		// For this test, we create engines that would behave differently if we had
		// whole word matching in the regex pattern generation
		testLine := "This is a test line with testing and test_func keywords"

		// Test with "test" - should match "test", "testing", and "test_func" in prefix mode
		// but only "test" and "test_func" in whole word mode (if implemented)
		eng1 := CreateSimpleEngine(t, "test", true, 4, 4)
		matches1 := eng1.IsLineMatch(testLine)

		// Current behavior: matches all occurrences of "test" as substring
		// Should find: "test" (position 10-14), "test" in "testing" (25-29), "test" in "test_func" (37-41)
		assert.Equal(t, 3, len(matches1), "Should find 3 matches for 'test' in current substring mode")
		assert.Equal(t, [][]int{{10, 14}, {25, 29}, {37, 41}}, matches1)

		// Test case 2: Word that only appears as standalone
		eng2 := CreateSimpleEngine(t, "line", true, 4, 4)
		matches2 := eng2.IsLineMatch(testLine)
		assert.Equal(t, 1, len(matches2), "Should find 1 match for 'line'")
		assert.Equal(t, [][]int{{15, 19}}, matches2)

		// Test case 3: Word that appears in compound forms
		testLine2 := "function func_call func() my_func"
		eng3 := CreateSimpleEngine(t, "func", true, 4, 4)
		matches3 := eng3.IsLineMatch(testLine2)

		// Current behavior: finds "func" in all contexts
		// Should find: "func" in "function" (0-4), "func" in "func_call" (9-13),
		// "func" in "func()" (19-23), "func" in "my_func" (29-33)
		assert.Equal(t, 4, len(matches3), "Should find 4 matches for 'func' in current substring mode")
		assert.Equal(t, [][]int{{0, 4}, {9, 13}, {19, 23}, {29, 33}}, matches3)

		// Test case 4: Multiple words test
		testLine3 := "class MyClass method classmethod my_method"
		eng4 := CreateSimpleEngine(t, "class method", true, 4, 8)
		matches4 := eng4.IsLineMatch(testLine3)

		// Should find the combination that satisfies both keywords within distance
		// This tests that the multi-keyword logic works correctly
		assert.Greater(t, len(matches4), 0, "Should find at least one match for 'class method' combination")

		// Test case 5: Demonstrate the need for whole word matching
		// Words that commonly cause false positives in code search
		falsePositiveTests := []struct {
			query string
			line  string
			desc  string
		}{
			{"in", "int main() { return 0; }", "should not match 'in' inside 'int'"},
			{"or", "for (int i = 0; i < 10; i++) {}", "should not match 'or' inside 'for'"},
			{"if", "diff --git a/file.txt b/file.txt", "should not match 'if' inside 'diff'"},
			{"id", "void method() { ... }", "should not match 'id' inside 'void'"},
			{"new", "renew subscription", "should not match 'new' inside 'renew'"},
		}

		for _, tt := range falsePositiveTests {
			// Test current substring matching behavior (should find false positives)
			substrEngine := CreateSimpleEngine(t, tt.query, true, 4, 4)
			substrMatches := substrEngine.IsLineMatch(tt.line)

			// Current implementation uses substring matching - should find matches
			assert.Greater(t, len(substrMatches), 0,
				"Non-whole-word matching should find matches for query '%s' in line '%s' (%s)",
				tt.query, tt.line, tt.desc)

			// Test whole word matching - should NOT find matches for false positive cases
			// Create a separate engine for whole word matching
			wholeWordEngine := NewSimpleContentSearchEngine(nil, 4, 4, true)
			err := wholeWordEngine.Compile(tt.query, true) // wholeWord = true
			assert.NoError(t, err, "Failed to compile whole word engine for query: %s", tt.query)

			wholeWordMatches := wholeWordEngine.IsLineMatch(tt.line)
			assert.Equal(t, 0, len(wholeWordMatches),
				"Whole word matching should NOT find matches for query '%s' in line '%s' (%s)",
				tt.query, tt.line, tt.desc)
		}

		// Test case 6: Cases where whole word matching should still match
		validWholeWordTests := []struct {
			query string
			line  string
			desc  string
		}{
			{"test", "run test suite", "should match standalone 'test'"},
			{"func", "func main() {", "should match 'func' at word boundary"},
			{"class", "public class MyClass {", "should match 'class' with boundaries"},
			{"var", "var x = 10;", "should match 'var' as keyword"},
		}

		for _, tt := range validWholeWordTests {
			// Test current substring matching behavior (should find matches)
			substrEngine := CreateSimpleEngine(t, tt.query, true, 4, 4)
			substrMatches := substrEngine.IsLineMatch(tt.line)
			assert.Greater(t, len(substrMatches), 0,
				"Non-whole-word matching should find matches for '%s' in '%s' (%s)",
				tt.query, tt.line, tt.desc)

			// Test whole word matching - should ALSO find matches for valid cases
			// Create a separate engine for whole word matching
			wholeWordEngine := NewSimpleContentSearchEngine(nil, 4, 4, true)
			err := wholeWordEngine.Compile(tt.query, true) // wholeWord = true
			assert.NoError(t, err, "Failed to compile whole word engine for query: %s", tt.query)
			...
			wholeWordMatches := wholeWordEngine.IsLineMatch(tt.line)
			assert.Greater(t, len(wholeWordMatches), 0,
				"Whole word matching should ALSO find matches for '%s' in '%s' (%s)",
				tt.query, tt.line, tt.desc)
		}
	})
}
