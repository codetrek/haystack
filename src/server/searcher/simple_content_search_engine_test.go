...
func TestContentLineMatch(t *testing.T) {
	...
	t.Run("Whole word matching - IsLineMatch behavior comparison", func(t *testing.T) {
		...
		for _, tt := range falsePositiveTests {
			eng := CreateSimpleEngine(t, tt.query, true, 4, 4)
			matches := eng.IsLineMatch(tt.line)

			// Document current behavior - these are cases where whole word matching would help
			if len(matches) > 0 {
				t.Logf("Current behavior for query '%s' in line '%s': found %d matches at %v (%s)",
					tt.query, tt.line, len(matches), matches, tt.desc)
			}
		}
		...
		for _, tt := range validWholeWordTests {
			eng := CreateSimpleEngine(t, tt.query, true, 4, 4)
			matches := eng.IsLineMatch(tt.line)
			assert.Greater(t, len(matches), 0, "Should find matches for '%s' in '%s' (%s)",
