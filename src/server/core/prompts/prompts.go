...
func savePrompt(batch pebble.Batch, workspace *workspace.Workspace, relPath string) {
	...
	if err != nil {
		log.Printf("Failed to read file %s: %v", relPath, err)
		return
	}
	...
	if workspace.EnablePromptSearch {
		...
		if err != nil {
			log.Printf("Failed to get embedding for file %s: %v", relPath, err)
			return
		}
		
		value, err := EncodeFloat32Vector(embedding)
		if err != nil {
			log.Printf("Failed to encode embedding for file %s: %v", relPath, err)
			return
		}
