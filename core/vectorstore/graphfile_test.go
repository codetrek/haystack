...
// It covers (1) a near-MaxUint64 poolOff whose poolOff+poolLen*8 wraps uint64 and
...
func TestGraphFile_CorruptHeaderRejected(t *testing.T) {
	...
	if err := corrupt(func(b []byte) {
		...
		binary.LittleEndian.PutUint64(b[poolOff:poolOff+8], 1<<20) // first POOL neighbor id
	}); err == nil {
