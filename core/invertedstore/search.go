...
//
// It is a type ALIAS of invertedindex.SearchResult (NOT a separate definition), so the store
// returns the IDENTICAL named type that invertedindex defines (the SearchResult struct is
// intentionally reused). invertedstore importing invertedindex is cycle-free: invertedindex does
// not import invertedstore. The shape is byte-identical (DocIds/WildDocIds map[int64]struct{} with
// the same json tags), so every existing SearchResult{...} literal and field access in this package
// compiles unchanged.
